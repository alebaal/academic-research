...
def pYL(gamma, r0, r2):
    return 2*gamma*H(r0,r2)*9.8692*10**4  

def pForce(gamma, rb, theta, fT):
    return ((forceGamma(gamma, rb, theta) - fT)/(np.pi*rb**2))*9.8692*10**5
...
def main():
    if (len(sys.argv) == 3):
        ...
        print("Creating graphic pressureXheight.pdf")
        ...
        for readFileTheoretical in sorted(glob.glob("./analysis/theoreticalData/resultASbridgek*.dat")):
            ...
            with open(readFileTheoretical,"r") as fileref:
                ...
                for line in reader_file:
                    ...
                    colY.append(float(line[6])*gamma*2*9.8692*10**4) # factor 10.0: convertion from \AA no nm
        ...
        for readFileMoviment in sorted(glob.glob("./analysis/8pk0.*/MeasurementsXheight/parameters-ASbridge-fitting-AllHeight.dat")):
            ...
            with open(readFileMoviment,"r") as fileref:
                ...
                for line in reader_file:
                    colX.append(float(line[0])*2) #h/2 -> h
                    colR0.append(ufloat(float(line[11]), float(line[12])))
                    colR2.append(ufloat(float(line[13]), float(line[14])))
                    colRb.append(ufloat(float(line[1]), float(line[2])))
                    colTheta.append(ufloat(float(line[3]), float(line[4])))
            with open("./analysis/8pk%s/MeasurementsXheight/AveForceLammps-AllHeight.dat" % (polarity),"r") as fileref:
                reader_file = csv.reader(fileref, delimiter='\t')   #fitting file has to skip one line
                next (reader_file)
                for line in reader_file:
                    colFt.append(ufloat(float(line[1]), float(line[2])))
            for i in range(len(colR0)):
                pressureYL = pYL(ufloat(gamma, ErrGamma), colR0[i], colR2[i])
                colY.append(pressureYL.n) 
                colYerr.append(pressureYL.s)
                pressureForce =  pForce(ufloat(gamma, ErrGamma), colRb[i], colTheta[i], colFt[i])
                colY2.append(pressureForce.n) 
                colY2err.append(pressureForce.s)
            ...
            plt.errorbar(colX, colY2, yerr = colY2err, fmt='o', markersize = 14, markeredgewidth = 2, capsize = 4 ,markerfacecolor='None', color= dicColorsPol[polarity], label = 'k = %s' % (polarity))
            colY    = []
            colYerr = []
            colY2    = []
            colY2err = []
            ...
            colTheta = []
            colFt = []
        plt.axis([10,55,-200,400])
        plt.ylabel('%s  %s%s%s' % ("Laplace Pressure ","[","atm","]"))
        ...
        plt.tight_layout()
        plt.savefig("./analysis/graphicsThermodynamic/pressureXheight.pdf")
        plt.close()
    else:
