...
def main():
    if (len(sys.argv) == 8):
        ...
        with open("./analysis/checkSlab/8pk%s/moviment-%s/profile/AS-bridge-profileDM-%s-0-%s.dat" % ( sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[5]), "r") as fileProfile:
        ...
        #Fitting
        if(polk < 0.35):
            profRadiusIni = R2*0.7 
            profRadiusEnd = R2*1.3
            neckRadiusIni = 0.95*colRadius[0]
            neckRadiusEnd = 1.05*colRadius[0]
            DeltaR0 = DELTA_R
            DeltaR2 = DELTA_R
        else:
            profRadiusIni = R2*0.9 
            profRadiusEnd = R2*1.1
        ...
        with open("./analysis/checkSlab/8pk%s/moviment-%s/fitting/ASbridge-fitting-k%s-moviment%s-%s-0-%s-rm%s.dat" % 
                           ( sys.argv[1], sys.argv[2], sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[5], sys.argv[7]), "w") as fileBestFit:
        ...
        with open("./analysis/checkSlab/8pk%s/moviment-%s/fitting/parameters-ASbridge-fitting-k%s-moviment%s-%s-0-%s-rm%s.dat" % 
                   ( sys.argv[1], sys.argv[2], sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[5], sys.argv[7]), "w") as fileBestFitParameters:
