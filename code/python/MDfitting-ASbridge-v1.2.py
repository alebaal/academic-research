#!/usr/bin/env python
#Here I have added the fitting for theta = 90
...
dicR_PROF_MIN_HPHILIC = {0.4:-50 , 0.5:-20, 0.6:-10}#  minimal radius of the hydrophilic profile
dicR_PROF_MAX_HPHILIC = {0.4:-1000, 0.5:-100, 0.6:-60}# maximal radius of the hydrophilic profile
...
def main():
    if (len(sys.argv) == 6):
        ...
        if(polk != 0.346):
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(MDprofile[1], MDprofile[0])              
            Z90plot = np.arange(0, dSurface, DELTAFITTINGRHO)
            R90plot = Z90plot*slope + intercept
            slopeError = std_err
            bestFitProfile = []
            bestFitProfile.append(list(R90plot))
            bestFitProfile.append(list(Z90plot))
            fittingErrorMin = compareProfiles(MDprofile, bestFitProfile)
            bestFitNeckRadius = ufloat(R90plot[0] , fittingErrorMin) 
            bestFitProfRadius = ufloat(99999999999, fittingErrorMin) 

            with open("./analysis/8pk%s/moviment-%s/fitting/parameters-ASbridge-LinearRegression-k%s-moviment%s-%s-0.dat" % (sys.argv[1], sys.argv[2], sys.argv[1], sys.argv[2], sys.argv[3]), "w") as fileBestFitParameters:
                 fileBestFitParameters.write("#1-slope\t2-slopeError\t3-intercept\n")
                 fileBestFitParameters.write("%f\t%f\t%f" % (slope, slopeError, intercept))
                 fileBestFitParameters.close()
            
        #Saving fitting profile
        with open("./analysis/8pk%s/moviment-%s/fitting/ASbridge-fitting-k%s-moviment%s-%s-0.dat" % ( sys.argv[1], sys.argv[2], sys.argv[1], sys.argv[2], sys.argv[3]), "w") as fileBestFit:
        ...
        if (polk != 0.346):
            ...
            if (slope > 0):
                Theta = 90 - umath.degrees(umath.atan(ufloat(slope,slopeError)))
            else:
                Theta = 90 + umath.degrees(umath.atan(-1*ufloat(slope,slopeError)))
            forceOverGammaBase = ((bestFitProfile[0][-1]*umath.sin(umath.radians(Theta))) - bestFitH*bestFitProfile[0][-1]**2)*2*np.pi	
            forceOverGammaNeck = (bestFitNeckRadius - bestFitH*bestFitNeckRadius**2)*2*np.pi
        ...
        print "tave"
        print "half height"
        print "Remove n Points close to interface"
