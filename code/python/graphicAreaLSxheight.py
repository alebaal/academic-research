...
print("Creating graphic AreaLSXheight.pdf")
...
for readFileTheoretical in sorted(glob.glob("./analysis/theoreticalData/resultASbridgek*.dat")):
    ...
    with open(readFileTheoretical,"r") as fileref:
        ...
        for line in reader_file:
            ...
            colY.append((2*np.pi*float(line[3])**2)/10**4) #1) Base Area  2)figure scale
...
for readFileMoviment in sorted(glob.glob("./analysis/8pk0.*/MeasurementsXheight/parameters-ASbridge-fitting-AllHeight.dat")):
    ...
    with open(readFileMoviment,"r") as fileref:
        ...
        for line in reader_file:
            colX.append(float(line[0])*2) #h/2 -> h
            colY.append(float(line[9])/10**4) #figure scale
            colYerr.append(float(line[10])/10**4)    #figure scale
        ...
        colYerr = []
        colX    = []
plt.axis([10,55,0.05,1.5])
#plt.axis([50,80,0.8,1.7])
plt.ylabel('%s  %s%s%s' % ("$\mathsf{A_{LS}}$  ","[","$\mathsf{10^4}$ $\mathsf{\AA^2}$","]"))
...
ax.text(50,0.3, "(a)", fontsize = 40)
ax.legend(loc='lower left', markerscale=0.8, borderpad = 0.2,labelspacing=0.3, handlelength=0.5 )
plt.tight_layout()
plt.savefig("./analysis/graphicsGeometry/AreaLSXheight.pdf")
plt.close()
