...
def main():
    if (len(sys.argv) == 3):
        ...
        print("Creating graphic ForceGammaXheight.pdf")
        ...
        for readFileTheoretical in sorted(glob.glob("./analysis/theoreticalData/resultASbridgek*.dat")):
            ...
            with open(readFileTheoretical,"r") as fileref:
                ...
                for line in reader_file:
                    ...
                    colY.append(forceGamma(gamma,float(line[3]), dicPolTheta[polarity])) # factor 10.0: convertion from \AA no nm
        ...
        for readFileMoviment in sorted(glob.glob("./analysis/8pk0.*/MeasurementsXheight/parameters-ASbridge-fitting-AllHeight.dat")):
            ...
            with open(readFileMoviment,"r") as fileref:
                ...
                for line in reader_file:
                    colX.append(float(line[0])*2) #h/2 -> h
                    forceTot = forceGamma(ufloat(gamma, ErrGamma), ufloat(float(line[1]), float(line[2])), ufloat(float(line[3]), float(line[4])))
                ...
                colYerr = []
                colX    = []
        plt.axis([10,55,0.55,1.5])
        plt.ylabel('%s  %s%s%s' % ("Surface Tension Force  ","[","nN","]"))
        ...
        ax.text(50,1.4, "(c)", fontsize = 40)
#        ax.legend(loc='upper right', markerscale=0.8, borderpad = 0.2,labelspacing=0.3, handlelength=0.5 )
        plt.tight_layout()
        plt.savefig("./analysis/graphicsThermodynamic/ForceGammaXheight.pdf")
        plt.close()
    else:
